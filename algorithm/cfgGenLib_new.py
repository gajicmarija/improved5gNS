...
def generate_ini(cfgName, baseCfgName, hostNumbers, clientsNumbers, hostIPs, bwSplits, rate, queueSize, withoutHTB = False, twoLevelHTB = False):
    # General config.
    print("in generate ini")
    #print(hostNumbers)
    ...
    # Routing and hosts.
            cfgString = cfgString + '*.configurator.config = xmldoc("configs/routing/%sroutingtest1.xml")\n' % cfgName
            for k, v in clientsNumbers.items():
                cfgString = cfgString + '*.n%s = %d\n' % (k, v)
            cfgString = cfgString + '\n'
            
            if not withoutHTB:
                # HTB.
                activeHosts = [k for k in hostNumbers.keys()] #if hostNumbers[k] >= 0 activeHosts is a list of all active applications
                ...
                cfgString = cfgString + '*.router*.ppp[0].queue.queue[*].dataCapacity = %db\n' %queueSize
                ...
                if not twoLevelHTB:
                    ...
                    r0pdf.append('[')
                    r1pdf.append('[')
                    for h in activeHosts: #for each application type
                        ips = generate_ip_addresses(hostIPs[h], hostNumbers[h])
                        #print(ips)
                        #print("-----------above are all the IPs-----------")

                        #here i need to check if the ip address is the last one, i.e there are no other clients in the network 

                        for i in range(0, hostNumbers[h]):#for all clients of that application type
                            ...
                            # if h == activeHosts[-2] and i==hostNumbers[h]-1: #if HVIP has clients make sure to set this index to -1
                            #     r0pdf.append('expr(ipv4.srcAddress.str() =~ "%s")' % ips[i])
                            #     r1pdf.append('expr(ipv4.destAddress.str() =~ "%s")' % ips[i]) 
                            # else:
                            #     print("in else ")
            ...
            # Rate and delay.
            cfgString = cfgString + '**.connFIX0.datarate = %dbps\n' % rate
            cfgString = cfgString + '**.connFIX0.delay = 10ms\n\n'

            # Write to file.
            if not withoutHTB:
                # myfile = open('../rndm/simulations/configs/routing/%sroutingtest1.xml' % cfgName, 'wb')
                # runfile = open('../rndm/simulations/run.txt', 'a')
                # print(cfgName)
                #print("appending")
                f = open('/Users/marijagajic/omnetpp-6.0/improved5gNS/simulations/omnetpp.ini', 'a')
            # else:
...
def generate_routing_xml(cfgName, hostNumbers, hostIPs, serverIPs, twoLevelHTB = False):
    ...
    print("opening file")
    myfile = open('/Users/marijagajic/omnetpp-6.0/improved5gNS/simulations/configs/routing/%sroutingtest1.xml' % cfgName, 'wb')
...
#   bwSplit:        Dict holding assured rate inbps per app type.
def generate_htb_xml(cfgName, hostNumbers, hostPrios, bwSplit):
    ...
    for h in activeHosts:
        ...
        qNum = qNum + 1
    xmldata = ET.tostring(configElem)
    myfile = open('/Users/marijagajic/omnetpp-6.0/improved5gNS/simulations/configs/htbTree/%s_htb.xml' % cfgName, 'wb')
    myfile.write(xmldata)

# Same as generate_htb_xml, but with per-flow leaves.
#ceilSplit is ceiling for the inner nodes
def generate_htb_xml_two_level(cfgName, hostNumbers, hostPrios, bwSplit,ceilSplit,leafBwSplit,leafCeilSplit):
    totalRate = sum(bwSplit.values())
    configElem = ET.Element('config')
    # FIXME: from collections import OrderedDict?
    # Root node.
    add_xml_subelement(
        ...
        {'type': 'class', 'id': 'root', 'fields': OrderedDict([
            ...
            ('quantum', ['int', 1500]), 
            ('mbuffer', ['int', 60])])})
    innerNodes = [k for k in bwSplit.keys() if bwSplit[k] > 0]
        ...
        for hostNum in range(0, hostNumbers[h]):
            add_xml_subelement(
                ...
                {'type': 'class', 'id': 'leafhost' + h + str(hostNum), 'fields': OrderedDict([
                    ...
                    ('rate', ['int', int(leafBwSplit[h]/ 1000)]),
                    ('ceil', ['int', int(leafCeilSplit[h] / 1000)]), 
                    ('burst', ['int', 2000]), 
                    ('cburst', ['int', 2000]), 
                    ('level', ['int', 0]), 
                    ('quantum', ['int', 1500]), 
...
def generate_all_config_files(cfgName, baseCfgName, hostNumbers, hostPrios, hostIPs, serverIPs, bwSplits, ceilSplits,leafBwSplit,leafCeilSplit, rate,routingNumbers, routingIPs,hostNumbersRouting, queueSize, useTwoLevelHTB):
    if not useTwoLevelHTB:
        # ini - for regular and anarcho version.
        print("In generate_all_config_files")
        f = open("../simulations/bufferStudiesVoD.txt", "a")
        print("open the file")
        f.write("./runAndExportHeatMaps.sh -i parameterStudyConfiguration.ini -c " + cfgName +  " -s 6 -n 1 \n")
        f.close()
        #print(hostNumbers)
        #generate_ini(cfgName, baseCfgName, hostNumbers,hostIPs, bwSplits,rate, queueSize)
        ...
        for bwSplit,ceilSplit in zip(bwSplits,ceilSplits):
            longName = "%s%s" % (cfgName, "".join(["%s%02d" % (k, round(v / 1e5)) for k, v in bwSplit.items()]))
            #generate_htb_xml(cfgName, hostNumbers, hostPrios, bwSplit, ceilSplit)
            #generate_routing_xml(cfgName, hostNumbersRouting,routingNumbers, routingIPs)
    else:
        print("In generate_all_config_files")
        f = open("../simulations/bufferStudiesVoD.txt", "a")
        print("open the file")
        ...
        generate_ini(cfgName, baseCfgName, hostNumbers, hostNumbersRouting,hostIPs, bwSplits,rate,queueSize,twoLevelHTB = True)
        
        
        
        #generate_ini('%s' % cfgName, baseCfgName, hostNumbers, hostIPs, bwSplits, ratewithoutHTB = True)
        # routing.xml - skip types with number == 0?
        # htb.xml (1 per bwSplit) - skip with number == 0
        for bwSplit,ceilSplit in zip(bwSplits,ceilSplits):
            longName = "%s%s" % (cfgName, "".join(["%s%02d" % (k, round(v / 1e5)) for k, v in bwSplit.items()]))
            generate_htb_xml_two_level(cfgName, hostNumbers, hostPrios, bwSplit, ceilSplit,leafBwSplit[0],leafCeilSplit[0]) #why is zero index needed for leaf splits
            generate_routing_xml(cfgName, hostNumbersRouting,routingNumbers, routingIPs, twoLevelHTB = True)
#useTwoLevel HTB is true when we have per flow quarantees
def generate_all_config_files_ceil(cfgName, baseCfgName, hostNumbers, hostPrios, hostIPs, serverIPs, bwSplits, ceilings,rate, queueSize, useTwoLevelHTB):
    if not useTwoLevelHTB:
        # ini - for regular and anarcho version.
        print("*************In generate_all_config_files_ceil")
        f = open("../simulations/bufferStudiesVoD.txt", "a")
        print("open the file")
        f.write("./runAndExportHeatMaps.sh -i parameterStudyConfiguration.ini -c " + cfgName +  " -s 6 -n 1 \n")
        f.close()
        generate_ini_ceil(cfgName, baseCfgName, hostNumbers, hostIPs, bwSplits,ceilings,rate,queueSize)
...
# def generate_ini_ceil(cfgName, baseCfgName, hostNumbers, hostIPs, bwSplits, ceiling,rate, queueSize, withoutHTB = False, twoLevelHTB = False):
#     # General config.
#     print("in generate_ini_ceil, queue size is ")
#     print(queueSize)
#     for bwSplit in bwSplits:
#         #longName = "%s%s" % (cfgName, "".join(["%s%02d" % (k, round(v / 1e5)) for k, v in bwSplit.items()]))
#         #print(longName)
...
#                 cfgString = cfgString + '*.router*.ppp[0].queue.queue[*].typename = "DropTailQueue"\n'
#                 # FIXME: rly?
#                 cfgString = cfgString + '*.router*.ppp[0].queue.dataCapacity = %db \n' % queueSize
#                 cfgString = cfgString + '*.router*.ppp[0].queue.htbHysterisis = false\n'
#                 xmlNames = ""
#                 # for bwSplit in bwSplits:
#                 #     longName = "%s_bw-%s" % (cfgName, "_".join(["%s_%02dk" % (k, round(v / 1e3)) for k, v in bwSplit.items()]))
...
def generate_ini_ceil(cfgName, baseCfgName, hostNumbers, hostIPs, bwSplits, ceiling, rate, queueSize, withoutHTB = False, twoLevelHTB = False):
    ...
    # Routing and hosts.
            ...
            if not withoutHTB:
                ...
                #                 r0pdf.append('expr(ipv4.srcAddress.str() =~ "%s"), ' % ips[i])
                #                 r1pdf.append('expr(ipv4.destAddress.str() =~ "%s"), ' % ips[i])    
            cfgString = cfgString + '*.router0.ppp[0].queue.classifier.packetFilters = [expr(ipv4.srcAddress.str() =~ "10.0.*.*"), expr(ipv4.srcAddress.str() =~ "10.1.*.*"), expr(ipv4.srcAddress.str() =~ "10.2.*.*"), expr(ipv4.srcAddress.str() =~ "10.3.*.*"), expr(ipv4.srcAddress.str() =~ "10.4.*.*"), expr(ipv4.srcAddress.str() =~ "10.5.*.*")]\n'
            cfgString = cfgString + '*.router1.ppp[0].queue.classifier.packetFilters = [expr(ipv4.destAddress.str() =~ "10.0.*.*"), expr(ipv4.destAddress.str() =~ "10.1.*.*"), expr(ipv4.destAddress.str() =~ "10.2.*.*"), expr(ipv4.destAddress.str() =~ "10.3.*.*"), expr(ipv4.destAddress.str() =~ "10.4.*.*"), expr(ipv4.destAddress.str() =~ "10.5.*.*")]\n\n\n' 
            ...
            f.close()
            #runfile.close()
# def generate_htb_xml_two_level(cfgName, hostNumbers, hostPrios, bwSplit,ceilSplit):
#     totalRate = sum(bwSplit.values())
