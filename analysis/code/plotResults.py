...
def mainEffectsPlot(testNames, numCli, queueSizesText, throughputs, granularities):
    ...
    axs[2].set_xlabel("Throughput (as % of the original="  + str(originalTPperClient[testName.split("a")[1]]) + "kbps for " + testName.split("a")[1]  + ")")
...
def plotEcdfQueueSizes(testNames):
    ...
    data.rename(columns = {' tp':'Throughput [% of the original]'}, inplace = True)
    ...
    granularities = [0, 1]
    throughputs = data["Throughput [% of the original]"].unique()
    ...
    for gran in granularities:
        for app in applications: 
            ...
            # Plot the CDF
            plt.rcdefaults()
            axes[granularities.index(gran)].step(values, cdf, where='post', label= app, linewidth=2, linestyle=linestyles[applications.index(app)])
            ...
            if gran == 0:
                ...
                granu = " Slice"
            axes[granularities.index(gran)].set_title("Granularity: " + granu, fontsize=17)
            axes[0].set_ylabel(ylabel="ECDF",fontsize=17)
            
    fig.savefig("/mnt/data/improved5gNS/analysis/exports/extracted/mos2/" + "ECDFqueueSizes" + ".png", bbox_inches='tight'  ) 

def plotFacetsBarPlots(testNames,keyword):

    plt.style.use('default')
    fig, axes = plt.subplots(4, 2, figsize=(12, 13.5))
    ...
    for app in applications: 
        for gran in granularities:
            ...
            for queueSize in queueSizes: 
                ...
                for tp in throughputs:
                    ...
                    df=df.loc[df['application'] == app]
                    df=df.loc[df["Throughput [% of the original]"] == tp]
            ...
            barsMOS.insert(loc=0, column="Throughput [% of the original]", value=pd.Series(throughputs))
            barsSTD.insert(loc=0, column="Throughput [% of the original]", value=pd.Series(throughputs))

            colors = ['#1f77b495', '#ff7f0e95', '#2ca02c95']
            textures = ["",'//', '\\' ]
            color_texture_map = {color: texture for color, texture in zip(colors, textures)}

            barsMOS.plot(x="Throughput [% of the original]", y=["Tiny Buffer","BDP", "10BDP"],color=colors, kind="bar",edgecolor='black',linewidth=0.3,yerr=barsSTD[["Tiny Buffer","BDP","10BDP"]].to_numpy().T, ax=axes[applications.index(app),gran],legend=False,width=0.5, capsize=0.5,error_kw={'elinewidth': 0.5, 'ecolor': 'black','capthick': 0.5},align='center') 
        ...
        fig.savefig("/mnt/data/improved5gNS/analysis/exports/extracted/mos2/" + "BarsFacetsV2ConfInt" + keyword +".png")  
...
def plotFacetsBarPlotsQuantizationFDO(testNames,keyword):
    ...
    for app in applications: 
        for gran in granularities:
            ...
            color_texture_map = {color: texture for color, texture in zip(colors, textures)}
            print(color_texture_map)
            barsMOS.plot(x="Throughput [% of the original]", y=["BDP"],color=colors, kind="bar",edgecolor='black',linewidth=0.3,yerr=barsSTD[["BDP"]].to_numpy().T, ax=axes,legend=True,width=0.5, capsize=0.5,error_kw={'elinewidth': 0.5, 'ecolor': 'black','capthick': 0.5},align='center') 
...
#The function plotFacetsBarPlotsSUvsNumCli(testNames) was used to show that system utilization does not depend on the number on clients due to linear scaling of the resources. See Figure 14 in paper.        
    ...
    for app in applications: 
        for gran in granularities:
            ...
            for number in numberOfClients: 
                SUforDifferentQueues = []
                STDforDifferentQueues= []
                
                for tp in throughputs:
                    # if queueSize == "Tiny Buffer" and gran==" F": 
                    #     #barsMOS.insert(loc=0, column=queueSize + str(tp), value=float("nan"))
                    #     SUforDifferentQueues.append(float("nan"))
                    #     STDforDifferentQueues.append(float("nan"))
                    #     #barsMOS.insert(loc=0, column=queueSize, value=statistics.mean(df[" MOS"]))
                    #     continue
    
                    df=pd.DataFrame()
                    df=data.loc[data['Number of clients'] == n]
                    df=df.loc[df['granularity'] == gran]
                    df=df.loc[df['application'] == app]
                    df=df.loc[df["Throughput [% of the original]"] == tp]
                    
                    SUforDifferentQueues.append(statistics.mean(df[" SU"])) 
                    mean, margin_of_error = calculate_confidence_interval(df[" SU"])
                    STDforDifferentQueues.append(margin_of_error)
                    
        
                barsMOS.insert(loc=0, column=number, value=pd.Series(SUforDifferentQueues))
                barsMOS.set_index(throughputs)
                barsSTD.insert(loc=0, column=number, value=pd.Series(STDforDifferentQueues))
                barsSTD.set_index(throughputs)
            ...
            colors = [cmap(i / (0.9*len(numberOfClients))) for i in range(len(numberOfClients))]    
         
            colors=['#1f77b4', '#ff7f0e', '#2ca02c']
            textures = ["//",'', '\\' ]

            color_texture_map = {color: texture for color, texture in zip(colors, textures)} #(x="Throughput [% of the original]", y=["Tiny Buffer","BDP", "10BDP"],color=colors, kind="bar",edgecolor='black',linewidth=0.2,yerr=barsSTD[["Tiny Buffer","BDP","10BDP"]].to_numpy().T, ax=axes[applications.index(app),gran],legend=False,width=0.6, capsize=0.5,error_kw={'elinewidth': 0.5, 'ecolor': 'black','capthick': 0.5},align='center') 
            barsMOS.plot(x="Throughput [% of the original]", y=["Tiny Buffer","BDP", "10BDP"],color=colors, kind="bar",edgecolor='black',linewidth=0.2,yerr=barsSTD[["Tiny Buffer","BDP","10BDP"]].to_numpy().T, ax=axes[applications.index(app),gran],legend=False,width=0.6, capsize=0.5,error_kw={'elinewidth': 0.5, 'ecolor': 'black','capthick': 0.5},align='center') 
            for bar in axes[applications.index(app),gran].patches:
                face_color_hex = '#{:02x}{:02x}{:02x}'.format(int(bar.get_facecolor()[0] * 255),
                                                            int(bar.get_facecolor()[1] * 255),
                                                            int(bar.get_facecolor()[2] * 255))
                bar.set_hatch(color_texture_map[face_color_hex])
            if applications.index(app) ==0:
                axes[applications.index(app),gran].title.set_text('Granularity: per ' + granularity)
            
            if applications.index(app) != len(applications)-1: 
                axes[applications.index(app),gran].tick_params(labelbottom=False)
                axes[applications.index(app),gran].set(xlabel=None)
                
            axes[applications.index(app),0].set(ylabel = "Link utilization \n " + app )
            axes[applications.index(app),gran].set_yticks([0,0.5,1])
            axes[applications.index(app),gran].grid(axis='y')
            axes[applications.index(app),gran].set_ylim(0,1.0)
            axes[applications.index(app),gran].set_axisbelow(True)
  
        lines_labels = [axes[0,0].get_legend_handles_labels()]
        lines, labels = [sum(lol, []) for lol in zip(*lines_labels)]
        fig.legend(lines, labels, title="Number of clients", loc='lower center', ncol=len(numberOfClients), fontsize=11.5)

        fig.savefig("/mnt/data/improved5gNS/analysis/exports/extracted/mos2/" + "BarsFacetsSUvsNumCliConfInt" +".png", bbox_inches='tight')  
...
def plotFacetsBarPlotsSUvsNumCli(testNames): 
    ...
    fig.savefig("/mnt/data/improved5gNS/analysis/exports/extracted/mos2/" + "BarsFacetsSUvsNumCliV8" +".png", bbox_inches='tight') 
...
def queueCost():
    ...
    plt.xlabel("Throughput [% of the original]")
...
def plotFacetsQueue():
    ...
    df.rename(columns = {' tp':'Throughput [% of the original]'}, inplace = True)
    ...
    test = sns.relplot(data=df, x= 'Throughput [% of the original]', y=' QU', hue = 'Queue size', col= "granularity", row=" app", kind = "line", markers=True ,height=1.27, aspect=2.5/1.27, lw=0.5,ci="sd",style="Queue size")
 test.set(ylim=(0, 1))
...
def plotHeatmapQueueSize():
    ...
    fig.text(0.5, 0.079, 'Throughput [% of the original ]', ha='center', fontsize=24) # + str(originalTPperClient[applications[0]]) + "kbps]"
...
def plotECDF_MOS_Variation():
    ...
    for app in applications:
        for queueSize in queueSizeText:
            ...
            linestyles = ['-', '-.', ':']      
            plt.plot(values, cdf,label=queueSizeTextFull[queueSizeText.index(queueSize)], linestyle=linestyles[queueSizeText.index(queueSize)],linewidth=2) #linestyles #label=queueSizeTextFull[queueSizeText.index(queueSize)] + app + granularity
...
def plotCDFnumCli_MOSbasic(): 
    ...
    for app in applications:
        ...
        for num in numCli:
            ...
            plt.plot(values, cdf,label= str(num), linestyle=linestyles[numCli.index(num)], linewidth=2) #label=queueSizeTextFull[queueSizeText.index(queueSize)] + app + granularity
            plt.xlabel('MOS', fontsize=17)
            plt.xlim(0,5)
            plt.ylabel('ECDF',fontsize=17)
            plt.tick_params(axis="both", labelsize=15)
        ...
        fig.savefig("/mnt/data/improved5gNS/analysis/exports/extracted/mos2/" + "cdfMOSnumCLIbasicV2" + app +".png",bbox_inches='tight', pad_inches=0.01)  
        plt.clf()
...
def plotCDF_gVsMOS():
    ...
    for app in applications:
        for g in granularity:
            ...
            plt.plot(values, cdf,label= "Granularity: per" + str(g)) 
            plt.xlim(1.00,4.5)
            plt.xlabel('MOS', fontsize=17)
            plt.ylabel('ECDF', fontsize=17)
    ...
    fig.savefig("/mnt/data/improved5gNS/analysis/exports/extracted/mos2/" + "ThickNumClicdfgMOSV3" + app +".png",bbox_inches='tight', pad_inches=0.01)  
...
def plotTpMOS35():  
    ...
    for queueSize in queueSizes: 
        ...
        for n in df['n'].unique():
            ...
            for index, row in dfProcessed.iterrows():
                print(row["Throughput [% of the original]"])
                print(row[" MOS"])
                if row[" MOS"] >= 3.5:
                    tpMOS35.append(row["Throughput [% of the original]"])
                    flagTP35 = 1
                    break
            if not flagTP35:
                print("appending NAN")
    ...
    ax.plot(x_positions[:11],bars["Tiny Buffer"], linestyle='dashed', zorder=5, label="_")
    ax.plot(x_positions[11:22],bars["BDP"], linestyle='dashed', zorder=5, label="_")
    ax.plot(x_positions[22:33],bars["10BDP"], linestyle='dashed', zorder=5, label="_")
   
    ax.set_ylabel("$Tp_{min}$", fontsize = 19)
...
def plotCorrelationMatrixAggregated(): 
    applications = ["General","hostVoD","hostLVD","hostVoIP","hostFDO"]
    fig, axs = plt.subplots(5,figsize=(18, 20))#
    type = "pearson"
    for app in applications: 
        print(app)
        df=pd.DataFrame()
        ...
        axs[0].tick_params(axis='x',labelsize=38,rotation=22.5)  #
        ...
        plt.tight_layout()
        if app != "General":
            axs[applications.index(app)].set_ylabel(app.replace("host",""),rotation=-90,labelpad=40)
        else:
            axs[applications.index(app)].set_ylabel(app,rotation=-90,labelpad=35)
        ...
        for (x, y), value in np.ndenumerate(df.corr(method=type).iloc[:-3,-3:]):
            if value > -0.59:
                axs[applications.index(app)].text(x, y, f"{value:.3f}", va="center", ha="center")
            else:
                axs[applications.index(app)].text(x, y, f"{value:.3f}", va="center", ha="center", color="white")

    cmap = plt.get_cmap('viridis')
    norm = mcolors.Normalize(vmin=-1, vmax=1)
    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
    sm.set_array([])  # You can set an empty array since it's not tied to any data
...
if __name__ == "__main__":
    ...
    ########
    plotCorrelationMatrixAggregated() #Figure 5 and 6 
    plotFacetsBarPlots(testNames," MOS") #use " SU" as a keyword to get a plot for system utilization, Figures 7 and 8 
    #plotECDF_MOS_Variation() #Figure 9 
    #plotFacetsBarPlotsQuantizationFDO(testNames," MOS") #Figure 10
    #plotCDFnumCli_MOSbasic() #Figure 11
    #plotTpMOS35() #Figure 12 
    #plotCDF_gVsMOS()#Figure 13 
    #plotFacetsBarPlotsSUvsNumCli(testNames) #Figure 14
    #plotEcdfQueueSizes(testNames) #Figure 18
